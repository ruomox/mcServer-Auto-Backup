#!/bin/bash
# mcServer Auto Backup script
# Ver 2.0

# ================= Configuration Section =================
# ================= 配置区域 =================

# Name of the screen session running the Minecraft server
# Minecraft 服务器所在的 screen 会话名称（用于发送指令）
SCREEN_NAME="YOUR_SCREEN_NAME"

# Root directory of the Minecraft server
# Minecraft 服务器根目录
SERVER_DIR="YOUR_SERVER_DIR"

# Directory where backup archives and state files are stored
# 备份文件与状态文件存放目录
BACKUP_DIR="YOUR_BACKUP_DIR"

# Backup log file (stdout + stderr will be appended here)
# 备份脚本运行日志（标准输出与错误输出）
BACKUP_LOG="$BACKUP_DIR/backup.log"

# World session lock file used by Minecraft to indicate active usage
# Minecraft 世界占用锁文件，用于判断世界是否正在被使用
SESSION_LOCK="$SERVER_DIR/world/session.lock"

# level.dat is used to detect world activity for cold backup checks
# level.dat 用于冷备份时判断世界是否处于静止状态
LEVEL_DAT="$SERVER_DIR/world/level.dat"

# Number of backup archives to keep (older ones will be deleted)
# 保留的备份文件数量（超过的旧备份将被清理）
# 建议保留更多份数，例如 7 天
KEEP_COUNT=3

# Minimum free disk space (GB) required to start a backup
# 触发备份前要求的最小磁盘空闲空间（GB）
MIN_FREE_GB=20        # At least 20GB free space / 至少保留 20GB 空闲

# Minimum free disk percentage required to start a backup
# 触发备份前要求的最小磁盘空闲百分比
MIN_FREE_PERCENT=5    # At least 5% free space / 至少保留 5% 空闲

# ---------------- save-all monitoring ----------------
# ---------------- save-all 监测配置 ----------------

# Minecraft latest.log path (used to monitor save-all completion)
# Minecraft latest.log 日志路径（用于检测 save-all 是否完成）
LOG_FILE="$SERVER_DIR/logs/latest.log"

# Log message indicating save-all finished successfully
# save-all 成功完成时的日志关键字
SAVE_SUCCESS_MSG="Saved the game"     # !!! NEED_TO_CHECK! / 需要确认！

# Maximum time to wait for save-all to complete (in seconds)
# 等待 save-all 完成的最大时间（秒）
# 300s is reasonable for large worlds (~10k chunks)
...
# zstd 压缩级别（1-19），数字越大压缩率越高但越慢
COMPRESS_LEVEL=3    # Speed-oriented preset / 速度优先

# Number of CPU threads used for compression
# 0 means auto (use all available cores)
# 压缩使用的 CPU 核心数，0 表示自动使用全部核心
COMPRESS_THREADS=0

# ---------------- Database backup ----------------
# ---------------- 数据库配置 ----------------

# Enable or disable MySQL database backup
# 是否启用 MySQL 数据库备份
...
# MySQL client authentication config file
# MySQL 客户端认证配置文件路径（推荐使用 .cnf）
...
# ---------------- BlueMap sync settings ----------------
# ---------------- BlueMap 配置 ----------------

# Enable rsync of BlueMap rendered web data
# 是否同步 BlueMap 网页渲染数据
# If you do not have any map Plugin like BlueMap, DO NOT ENABLE!
# 如果你没有 BlueMap 等在线地图插件，不要启用！
ENABLE_BLUEMAP_RSYNC=true   # true / false

# If true, BlueMap data will be ![EXCLUDED] from tar archive
# 注意：若未排除，BlueMap 数据将被打包进历史备份文件
# 压缩阶段是否 ![排除] bluemap 目录
TAR_EXCLUDE_BLUEMAP=true

# Sync BlueMap data once every N backups
# BlueMap 同步周期（每 N 次备份同步一次）
BLUEMAP_SYNC_INTERVAL=7

# File used to persist BlueMap sync counter
# BlueMap 同步计数器文件（用于跨运行保存状态）
